            ...
            else 
            {
                ...
                if (move_uploaded_file($_FILES["filetoupload"]["tmp_name"], $target_file)) 
                {
                    ...
                    if (mysqli_query($con, $query)) 
                    {
                        ...
                        $queryemployee = "SELECT * FROM `b_usermaster` WHERE `emp_email`='" . $_SESSION['directreportingto'] . "' AND `supportregion`='" . $_SESSION['supportregion'] . "'";
						$resultofup = mysqli_query($con, $queryemployee);
							$drtemail = mysqli_fetch_array($resultofup, MYSQLI_ASSOC);
                            $email1 = $drtemail['emp_email'];
							$name1 = $drtemail['emp_name'];
